	...
	*/
	int contN() {
		...
		if (esVacio()) {
			...
			contarNodos(_ra, cont);
			return cont;
		}
	}

	/*
	* Cuenta altura del árbol
	*/
	int contA() {
		if (esVacio()) {
			return 0;
...
private:
	...
	int contarHojas(Arbin ar) {
		if (ar.esVacio()) {
			return 0;
		}

		if ((ar.hijoDr().esVacio()) && (ar.hijoIz().esVacio())) {
			return 1;
		}
		else
		{
			return (contarHojas(ar.hijoIz()) + contarHojas(ar.hijoDr()));
		}
	}

	void contarNodos(Arbin ra, int& contador) {
		if (!ra.esVacio()) {
			contador++;
			contarNodos(ra.hijoIz(), contador);
			contarNodos(ra.hijoDr(), contador);
		}
	}

	int contarAltura(Arbin ra){
		int a = 0, b = 0;

		if ((ra.hijoDr().esVacio()) && (ra.hijoIz().esVacio())) {
			return 1;
		}
		else
		{
			if (!ra.hijoIz().esVacio()) {
				a = contarAltura(ra.hijoIz());
			}
			if (!ra.hijoDr().esVacio()) {
				b = contarAltura(ra.hijoDr());
			}
			if (a < b) 
				return b + 1;
			else 
