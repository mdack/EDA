...
bool SetOfInts3::operator==(const SetOfInts3& set){
	...
	if (set.size != this->size) enc = false;
	else {
		while (i < size && enc) {
			if ((elems[i]) != (set.elems[i])) enc = false;
			i++;
		}
	}
	return enc;
...
/* La complejidad de este algoritmo sería O(log n) */
bool SetOfInts3::operator<(const SetOfInts3 & set)
{
	...
	if (set.size < this->size) {
		binSearch(set.elems[0], esSubconjunto, p);
		
		if (esSubconjunto) {
			p++;
			while (esSubconjunto && (p < size) && (cont < set.size)) {
				if (elems[p] > set.elems[cont]) esSubconjunto = false;
				
				if (elems[p] == set.elems[cont]) {
					cont++;
				}

				p++;
			}
			if (cont != set.size) esSubconjunto = false;
		}
	}
